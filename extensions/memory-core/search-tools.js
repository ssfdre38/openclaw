...
export function registerMemorySearchTools(api) {
    /**
    ...
    api.registerTool((ctx) => {
        ...
        return {
            ...
            description: "Search across daily memories (last 7 days), archived memories (90-day retention), and evergreen memories (permanent). Returns top matching highlights with tier tags. Use for recalling past conversations, decisions, preferences, or facts. Token-efficient: returns summaries, not full transcripts.",
            ...
            execute: async (_toolCallId, params) => {
                try {
                    ...
                    const config = {
                        tiers: params.tiers || ["daily", "archived", "evergreen"],
                    ...
                    const limitedResults = applyTokenBudget(results.slice(0, config.maxResults), config.maxTokens || 10000);
                    return {
                        success: true,
    ...
    api.registerTool((ctx) => {
        ...
        return {
            ...
            description: "Search ONLY evergreen (permanent) memories: birthdays, instructions, preferences, technical facts, relationships. Fast lookup for critical long-term information. Does not search daily or archived memories.",
            parameters: Type.Object({
    ...
    api.registerTool((ctx) => {
        ...
        return {
            ...
            description: "Retrieve full daily memory file for a specific date. Returns all highlights from that day. Use when you need complete context for a specific day, not just search results.",
            parameters: Type.Object({
    ...
    api.registerTool((ctx) => {
        ...
        return {
            ...
            description: "Retrieve full archived memory file for a specific date (7-90 days old). Returns all highlights from that day. Use for deeper historical context.",
            parameters: Type.Object({
                date: Type.String({ description: "Date to retrieve (YYYY-MM-DD)" }),
            }),
            execute: async (_toolCallId, params) => {
...
function formatEvergreenForSearch(item) {
    if (item.instruction)
        return `Instruction: ${item.instruction}`;
    if (item.preference)
        return `Preference: ${item.preference}`;
    if (item.fact)
        return `Fact: ${item.fact}`;
    if (item.name)
        return `Relationship: ${item.name} (${item.role})`;
    if (item.person)
        return `Date: ${item.person} - ${item.type} on ${item.date}`;
    return JSON.stringify(item);
}
function formatEvergreenForDisplay(item) {
    if (item.instruction) {
...
function detectEvergreenCategory(item) {
    if (item.instruction)
        return "instructions";
    if (item.preference)
        return "preferences";
