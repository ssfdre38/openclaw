...
/**
 * 3-Tier Memory System: Dailies Manager
 * 
 * Manages daily memory files with:
 * - ISO timestamps (2026-03-06T16:50:00.000Z)
 * - User ID tracking
 * - UUIDs for each highlight
 * - Structured JSON format
 */

export interface DailyHighlight {
  id: string; // hl-YYYY-MM-DD-NNN
...
export interface MemoryIndex {
  ...
  dailies: {
    ...
    currentlyLoaded: string[]; // Dates of currently loaded dailies
  };
  archived: {
    files: Array<{
      date: string;
  ...
  topicIndex: Record<string, string[]>; // topic -> [dates]
}

export class DailiesManager {
  private memoryPath: string;
  private dailiesPath: string;
   ...
   */
  private getArchivedPath(date: string): string {
   ...
   */
  private generateHighlightId(date: string, existingIds: string[]): string {
   ...
   */
  async writeDailyMemory(memory: DailyMemory): Promise<void> {
   ...
   */
  async readDailyMemory(date: string): Promise<DailyMemory | null> {
   ...
   */
  async appendHighlights(
    date: string,
    sessionId: string,
    newHighlights: Omit<DailyHighlight, "id">[],
  ): Promise<void> {
    ...
    const highlightsWithIds: DailyHighlight[] = newHighlights.map((h) => ({
      ...h,
   ...
   */
  async moveDailyToArchived(date: string): Promise<void> {
   ...
   */
  getLoadableDates(daysToLoad: number): string[] {
    const dates: string[] = [];
   ...
   */
  async loadRecentDailies(daysToLoad: number): Promise<DailyMemory[]> {
    ...
    if (this.index) {
      this.index.dailies.currentlyLoaded = dates.filter((date) =>
        this.index!.dailies.files.some((f) => f.date === date)
      );
      await this.saveIndex();
    }
   ...
   */
  async searchDailiesByTopic(topic: string): Promise<DailyMemory[]> {
    if (!this.index) await this.initialize();
    if (!this.index) return [];
