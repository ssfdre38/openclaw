...
export class MemoryLoader {
    ...
    evergreenManager;
    daysToLoad;
    constructor(memoryPath, daysToLoad = 7) {
    ...
    async loadMemories() {
        ...
        // Calculate stats
        const evergreenCount = evergreen.dates.length +
    ...
    formatForContext(memories) {
        ...
        if (memories.evergreen.instructions.length > 0) {
            ...
            memories.evergreen.instructions.forEach((item) => {
                sections.push(`- [${item.id}] ${item.instruction}`);
                if (item.context)
                    sections.push(`  Context: ${item.context}`);
            });
            sections.push("");
        }
        if (memories.evergreen.relationships.length > 0) {
            sections.push("## Relationships & Community");
            memories.evergreen.relationships.forEach((item) => {
        ...
        if (memories.evergreen.technicalFacts.length > 0) {
            ...
            memories.evergreen.technicalFacts.forEach((item) => {
        ...
        if (memories.evergreen.preferences.length > 0) {
            ...
            memories.evergreen.preferences.forEach((item) => {
        ...
        if (memories.evergreen.dates.length > 0) {
            ...
            memories.evergreen.dates.forEach((item) => {
            ...
            for (const day of memories.recentDailies) {
                ...
                for (const highlight of day.highlights) {
                    ...
                    if (highlight.context)
                        sections.push(`  Context: ${highlight.context}`);
                    if (highlight.topics.length > 0)
                        sections.push(`  Topics: ${highlight.topics.join(", ")}`);
                }
                sections.push("");
            }
        }
 ...
 */
export function registerMemoryLoader(api) {
