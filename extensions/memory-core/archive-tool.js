 ...
 */
export function registerArchiveTool(api) {
    api.registerTool((ctx) => ({
        ...
        execute: async (_toolCallId, params) => {
            try {
                const memoryPath = path.join(ctx.workspaceDir || process.env.OPENCLAW_DIR || ".", "memory");
                const dailiesManager = new DailiesManager(memoryPath);
                await dailiesManager.initialize();
                // Get date (default to today)
                const date = params.date || new Date().toISOString().split("T")[0];
                const highlights = (params.highlights || []).map(h => ({
